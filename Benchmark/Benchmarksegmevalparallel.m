...
[encimagevideo,encisvideo]=Collectimagevideowithnames(iids,opts.tempConsistency);
...
parfor i=1:numel(encimagevideo)
  ...
  if ( (any(strcmp(metrics,'bdry'))) || (any(strcmp(metrics,'all'))) )
    use3dbdry=false;
    Evaluatesegmbdry(inFile,gtFile, evFile1, opts.nthresh, opts.maxDist, opts.thinpb, use3dbdry, excludeth, offsetgt);
  end
  %Evaluate 3D boundary benchmark
  %     if ( (any(strcmp(metrics,'3dbdry'))) || (any(strcmp(metrics,'all'))) )
  %         use3dbdry=true;
  %         Evaluatesegmbdry(inFile,gtFile, evFile5, opts.nthresh, opts.maxDist, opts.thinpb, use3dbdry, excludeth, offsetgt);
  %     end
  ...
  if ( (any(strcmp(metrics,'regpr'))) || (any(strcmp(metrics,'sc'))) || (any(strcmp(metrics,'pri'))) || (any(strcmp(metrics,'vi'))) || (any(strcmp(metrics,'all'))) )
    Evaluatesegmregion(inFile, gtFile, evFile2, evFile3, evFile4, evFile6, opts.nthresh, metrics, excludeth, offsetgt);
  end
end
fprintf('\n');
